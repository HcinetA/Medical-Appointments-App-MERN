...
router.post('/', patientChecker.savePatientCheck, async (req, res) => {
  try {
    ...
    if (already_exists_patient.length > 0) {
      return res
        .status(400)
        .json('Un autre patient possede le meme numero de telephone ');
    } else {
...
router.delete('/:patient_id/:appointment_id', async (req, res) => {
  try {
    ...
    const appointments = patient.appointments.find(
      (app) => app._id === req.params.appointment_id
    );
    if (!appointments) {
      return res.status(400).json('rendez-vous inexistant');
    }

    patient.appointments = patient.appointments.filter(({ _id }) => {
